...
def hello_arm(config):
    ...
    try:
        with bosdyn.client.lease.LeaseKeepAlive(lease_client):
            ...
            gripper_command = RobotCommandBuilder.claw_gripper_open_fraction_command(0.15)
            ...
            robot.logger.info('Moving arm to pregrasp pose')
            ...
            # Send the request
            cmd_id = command_client.robot_command(command)
            robot.logger.info('Moving arm to grasp pose')
            ...
            # Send the request
            cmd_id = command_client.robot_command(command)
            robot.logger.info('Closing gripper on drawer')
            time.sleep(1)

            # Move the arm to a different position
            hand_ewrt_flat_body.x = 0.75
            ...
            # Send the request
            cmd_id = command_client.robot_command(command)
            robot.logger.info('Pulling drawer open')

            # Wait until the arm arrives at the goal.
            # Note: here we use the helper function provided by robot_command.
            block_until_arm_arrives(command_client, cmd_id)

            robot.logger.info('Drawer open')
            time.sleep(1)
            ...
            # Send the request
            cmd_id = command_client.robot_command(command)
            robot.logger.info('Opening hand')
            time.sleep(2)

            # Move the arm to a different position
            hand_ewrt_flat_body.x = 0.6
            ...
            # Send the request
            cmd_id = command_client.robot_command(command)
            robot.logger.info('Hand is safe location')
